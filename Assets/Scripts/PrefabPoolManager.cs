...
public class PrefabPoolManager : NetworkBehaviour
    {
        ...
        void Start()
        {
            ...
            ClientScene.RegisterPrefab(prefab2, SpawnHandler, UnspawnHandler);
        }

        void OnDestroy()
        {
            ClientScene.UnregisterPrefab(prefab);
            ClientScene.UnregisterPrefab(prefab2);
        }
        ...
        GameObject CreateNew2()
            {
                ...
                // use this object as parent so that objects dont crowd hierarchy
                GameObject next = Instantiate(prefab2, transform);
                next.name = $"{prefab2.name}_pooled_{currentCount}";
                next.SetActive(false);
                currentCount++;
                return next;
            }
        ...
        public GameObject GetFromPool2(Vector3 position, Quaternion rotation)
            {
                GameObject next = pool.Count > 0
                ? pool.Dequeue() // take from pool
                : CreateNew2(); // create new because pool is empty
